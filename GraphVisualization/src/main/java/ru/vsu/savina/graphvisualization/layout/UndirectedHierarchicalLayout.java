...
public class UndirectedHierarchicalLayout implements ILayout<UndirectedGraph> {
    @Override
    public IGraphWrapper layout(UndirectedGraph graph) {
        ...
        for (int i = 0; i < components.size(); i++) {
            List<List<Integer>> subgraphAdjList = HierarchicalLayout.createDirectedAdjustmentList(components.get(i), graph.getEdgeList());
