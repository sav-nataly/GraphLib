...
public class DepthFirstTraversalUndirectedAlgorithm implements IGraphTraversalAlgorithm {
    @Override
    public List<UndirectedVertex> execute(IGraph graph, IVertex startVertex) {
        ...
        while (!q.isEmpty()) {
            UndirectedVertex vertex = q.removeFirst();
            ...
            if (!used.contains(vertex)) {
                ...
                for (Map.Entry<UndirectedEdge, EdgeType> entry : vertex.getEdgeMap().entrySet()) {
                    ...
                    if (!used.contains(entry.getKey().getSource()))
                        q.addFirst(entry.getKey().getSource());
                }
            }
        }
